...
function dateStringToTimeAgo(dateString) {
  ...
  if (seconds < 60) {
    return "ahora"; //just now
  } else if (minutes < 60) {
    return `hace ${minutes} minutos`; // ${minutes}m ago
  } else if (hours < 24) {
    return `hace ${hours} horas`; // ${hours}h ago
  } else if (days < 7) {
    return `hace ${days} días`; // ${days}d ago
  } else {
    return `hace ${weeks} semanas`; // ${weeks}w ago
  }
}
/**

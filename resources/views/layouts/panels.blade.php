...
<!-- User panel -->
<section x-show="isUserPanelOpen" x-transition:enter="transition duration-300 ease-in-out transform" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition duration-300 ease-in-out transform" x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full" @keydown.escape="window.innerWidth <= 1024 ? isUserPanelOpen = false : ''" tabindex="-1" x-ref="userPanel" class="fixed inset-y-0 top-0 right-0 z-10 flex-shrink-0 bg-white xl:z-0 w-80 dark:bg-darker dark:text-light xl:border-l dark:border-indigo-800 focus:outline-none">
  <h2 class="sr-only">User panel</h2>
  <!-- Close button -->
  <div class="absolute left-0 p-2 transform -translate-x-full">
    <button @click="isUserPanelOpen = false" class="p-2 rounded-md text-dark dark:text-light focus:outline-none focus:ring">
  ...
  <div class="flex flex-col h-screen">
    ...
    <div class="flex-1 p-4 space-y-8 overflow-y-hidden hover:overflow-y-auto">
      ...
      <div class="flex flex-col items-center space-y-2">
        <!-- User avatar -->
        <img class="w-20 h-20 rounded-full dark:opacity-70" src="{{Auth::user()->profile_photo_url}}" alt="{{ Auth::user()->name }}" />
      ...
      <div class="space-y-6">
        ...
        <div class="space-y-4">
          <a href="#" class="flex items-start space-x-2 group">
            ...
            <span class="text-xs text-gray-500 whitespace-nowrap dark:text-indigo-500">1h
              ago</span>
          </a>
          <a href="#" class="flex items-start space-x-2 group">

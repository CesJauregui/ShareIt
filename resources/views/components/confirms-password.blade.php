...
<x-dialog-modal wire:model.live="confirmingPassword">
    ...
    <x-slot name="content">
        ...
        <div class="mt-4" x-data="{}" x-on:confirming-password.window="setTimeout(() => $refs.confirmable_password.focus(), 250)">
            <x-input type="password" name="password" id="password" class="mt-1 block w-3/4" placeholder="{{ __('Password') }}" autocomplete="current-password"
    ...
    <x-slot name="footer">
        ...
        <x-button class="ml-3" dusk="confirm-password-button" wire:click="confirmPassword" wire:loading.attr="disabled">
            ...
            <x-slot:buttonName>{{ $button }}</x-slot>
        </x-button>
    </x-slot>
</x-dialog-modal>
@endonce
